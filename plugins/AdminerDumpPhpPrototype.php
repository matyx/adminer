...
class AdminerDumpPhpPrototype {
	...
	private $formats = [
		...
		'code-class-php8' => 'Data Class PHP8',
	];
	...
	public function dumpTable($table) {
		...
		else if($_POST['format'] == 'code-form') {
			ob_start();
			$this->exportAsForm($table);
			$this->printCode(ob_get_clean());

		}
		else if($_POST['format'] == 'code-class') {
			ob_start();
			$this->exportAsDataClass($table, false);
			$this->printCode(ob_get_clean());
		}
		else if($_POST['format'] == 'code-class-php8') {
			ob_start();
	...
	private function exportAsForm($table) {
		foreach(fields($table) as $field => $info) {
			...
			else if($type === 'int' && strpos($field, '_id')) {
			...
			else if($type === 'string' && strpos($info['type'], 'text') === false) {
