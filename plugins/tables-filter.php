...
class AdminerTablesFilter {
	function tablesPrint($tables) { ?>
		...
		<ul id='tables'>
			...
			$select = lang('select');
			$structure = lang('Show structure');
			foreach($tables as $table => $status) {
				echo '<li data-table-name="' . h($table) . '"><a href="' . h(ME) . 'select=' . urlencode($table) . '"' . bold($_GET["select"] == $table || $_GET["edit"] == $table, "select") . ">" . $select . "</a> ";
				$name = h($status["Name"]);
